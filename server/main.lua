...
RegisterNetEvent('offroad:server:requestStart', function(trailId)
    ...
    if #(coords - trail.start) > Config.StartDistance then
...
RegisterNetEvent('offroad:server:finishRun', function(trailId)
    ...
    local coords = GetEntityCoords(ped)
    if #(coords - trail.finish) > Config.FinishDistance then
    ...
    if existing then
        ...
        MySQL.update.await([[
            ...
            SET player_name = ?, time_ms = ?, created_at = NOW()
            WHERE id = ?
...
RegisterNetEvent('offroad:server:saveTrail', function(data)
    ...
    if not isAdmin(src) then
        notify(src, 'You do not have permission to save trails.', 'error')
        return
    end
